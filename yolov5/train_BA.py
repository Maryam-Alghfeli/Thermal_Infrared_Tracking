...
def parse_opt(epochs,batch_size,known=False):
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', type=str, default=ROOT / 'yolov5s.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / '/home/nouf.alshamsi/AI702/Project/FLIR_TII/TII_FLIR.yaml', help='dataset.yaml path')
...
def generate_bee(index):
    ...
    population[index][1] = int(random.uniform(batch_start, batch_end))

def evaluate(bee): ##need edit
    # train NN and calculate the accuracy
    #model = lstm_module.LSTM(num_epochs=int(bee[0]), num_units=int(bee[1]))#we should edit here by writing 
    # with open("opt.yaml", 'rw') as f:#add your path
    #     doc = yaml.load(f)
    # doc['epochs'] = bee[0]
    # doc['batch_size'] = bee[1]
    epochs=int(bee[0])
    batch_size=int(bee[1])
    opt = parse_opt(epochs,batch_size,False)
    main(opt)
    # subprocess.run(["python", "train.py"])
    global tt
    tt+=1
    if(tt==1):
        s='/home/maryam.alghfeli/Desktop/Project_Final/home/nouf.alshamsi/AI702/Project/yolov5/runs/train/exp/results.csv'
    else:
        s='/home/maryam.alghfeli/Desktop/Project_Final/home/nouf.alshamsi/AI702/Project/yolov5/runs/train/exp'+str(tt)+'/results.csv'
...
def BA():
    ...
    global nep
    nep= 1  # number of bees around the elite bees
    global nsp
    nsp= 1  # number of bees around the good bees
    global ngh
    ...
    for i in range (5):
        for k in range(e):
            population[k]=local_search(population[k], '', nep)
        for j in range(e,m):
            population[j]=local_search(population[j], '', nsp)
